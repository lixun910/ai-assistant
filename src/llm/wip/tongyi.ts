...
interface TongyiMessage {
  ...
  content: string;
  tool_calls?: BindToolsInput[];
}

export interface ChatTongyiCallOptions extends BaseChatModelCallOptions {
  /**
...
interface AlibabaTongyiChatInput {
  ...
  /** The result format: use message format for tools */
  resultFormat?: 'text' | 'message';

  /** Messages to pass as a prefix to the prompt */
  prefixMessages?: TongyiMessage[];

  /** API Url for calling qwen model */
  apiUrl?: string;
 ...
 * ```
 */
export class CustomChatAlibabaTongyi
  ...
  implements AlibabaTongyiChatInput
{
  ...
  resultFormat?: 'text' | 'message' = 'text';
  ...
  constructor(
    ...
    this.resultFormat = fields.resultFormat ?? 'text';
  }
  ...
  ): Runnable<BaseLanguageModelInput, AIMessageChunk, ChatTongyiCallOptions> {
    return this.bind({
      // @ts-ignore FIX TYPE Error here
  ...
  invocationParams(): ChatCompletionRequest['parameters'] {
    const parameters: ChatCompletionRequest['parameters'] = {
      ...
      result_format: this.resultFormat,
  ...
  async _generate(
    ...
    const data = parameters?.stream
      ...
      : await this.completionWithRetry(
          ...
          false,
          options?.signal
        ).then<ChatCompletionResponse>((data: any) => {
          if (data?.code) {
  ...
  async _getTongyiStream(parameters, messages, options) {
  ...
  _createAIMessageFromStreamChunk(chunk) {
    // message_format is 'message'
    if ('choices' in chunk.output) {
      if (chunk.output.choices && chunk.output.choices.length > 0) {
        const { message, finish_reason } = chunk.output.choices[0];
  ...
  async *_streamResponseChunks(
    ...
    const messagesMapped: TongyiMessage[] = messages.map((message) => ({
      role: messageToTongyiRole(message),
      content: message.content as string,
      // @ts-ignore need to convert messages to TongyiMessages, so it supports tool_calls
      ...(message.tool_calls ? { tool_calls: message.tool_calls } : {}),
    }));

    const stream = await this._getTongyiStream(
      parameters,
      messagesMapped,
      options
    );
    ...
    for await (const chunk of stream) {
      const rst = this._createAIMessageFromStreamChunk(chunk);
      let text = '';
      let finish_reason = null;

      if (rst) {
        text = rst.message.content as string;
        finish_reason = rst.finish_reason;
      }

      yield new ChatGenerationChunk({
        text,
        message: rst ? rst.message : new AIMessageChunk({ content: text }),
        generationInfo:
          finish_reason === 'stop'
            ? {
                finish_reason,
  ...
  protected async *createTongyiStream(
    request: ChatCompletionRequest,
    signal?: AbortSignal
  ) {
    const response = await fetch(this.apiUrl, {
...
/**
 * Wrapper around Ali QwenVL for vision
 */
export class ChatAlibabaQwenVL extends CustomChatAlibabaTongyi {
  static lc_name() {
    return 'ChatAlibabaQwenVL';
  }

  constructor(fields: Partial<AlibabaTongyiChatInput> = {}) {
    super({
      ...fields,
      model: fields.model ?? 'qwen-vl-plus',
      apiUrl:
        fields.apiUrl ??
...
/**
 * Wrapper around Ali Qwen-voice for voice
 */
export class ChatAlibabaQwenAudio extends CustomChatAlibabaTongyi {
  static lc_name() {
    return 'ChatAlibabaQwenAudio';
  }

  constructor(fields: Partial<AlibabaTongyiChatInput> = {}) {
    super({
      ...fields,
      model: fields.model ?? 'qwen2-audio-instruct',
      apiUrl:
        fields.apiUrl ??
